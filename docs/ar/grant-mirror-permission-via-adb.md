...
وهذا الإذن أيضًا شرط لازم لميزة **نسخ هذا التطبيق فقط** (Android 15+)، إذ لا تعمل بدونه.
...
- **الإذن يُعاد ضبطه بعد إعادة التشغيل** — على بعض الأنظمة (مثل MIUI/HyperOS) لا تبقى أذونات `appops` بعد إعادة التشغيل. أعد تنفيذ الأمر بعد كل إعادة تشغيل؛ يوفّر عليك ADB عبر Wi-Fi (تصحيح الأخطاء اللاسلكي) توصيل الكابل في كل مرة، لكنه لا يمنع إعادة ضبط الإذن.
