...
일반적인 Android API로는 할 수 없는 일을 가능하게 합니다. **[Shizuku](https://shizuku.rikka.app/) 또는 루트**가 필요하며, 전적으로 선택 사항입니다. 둘 다 없는 휴대전화에서는 **달라지는 것이 없습니다** — 이 섹션은 설정에 그대로 표시되지만 그 안의 항목은 모두 비활성화되어 있고, 나머지 기능은 지금까지와 똑같이 동작합니다.
...
> [!IMPORTANT]
> **ADB로 시작한 Shizuku 서버는 차량에 연결하면 종료됩니다.** USB 연결은 휴대전화를 액세서리 모드로 전환하는데, 이때 ADB가 다시 시작되면서 Shizuku도 함께 종료됩니다 — 무선 디버깅도 같은 ADB를 사용하므로 도움이 되지 않습니다. Shizuku를 다시 시작하면 ScreenOnAuto가 알아서 다시 연결됩니다. 더 편한 방법은 watchdog 서비스가 대신 다시 시작해 주는 [thedjchi 포크](https://github.com/thedjchi/Shizuku)를 사용하는 것입니다. root 사용자와 무선 Android Auto 연결은 영향을 받지 않습니다.

> **화면을 다시 켜려면:** 터치스크린도 패널과 함께 꺼지므로 휴대전화를 만져도 반응하지 않습니다. 차량
> 화면의 **자동 화면 어둡게** 버튼을 누르세요 — 자동 화면 어둡게를 켜거나 끄면 화면이 깨어납니다(먼저
> **화면 버튼 → 기능 버튼**에서 이 버튼을 켜 두어야 하며, 기본값은 꺼짐입니다). 또는 휴대전화의 전원
> 버튼을 **두 번** 누르세요. 첫 번째 누름이 시스템을 절전 상태로 보내기 때문입니다. 미러링을 중지하거나
> Android Auto 연결을 해제해도 화면이 다시 켜집니다.
